...
<body class="bg-graybg">
    ...
    <div class="min-h-screen">
        {{-- SIDEBAR --}}
        <aside class="bg-linear-to-br from-violet-600/80 to-gray-900 -translate-x-80 fixed inset-0 z-50 my-4 ml-4 h-[calc(100vh-32px)] w-72 rounded-xl transition-transform duration-300 xl:translate-x-0 shadow-xl">
            {{-- HEADER SIDEBAR --}}
                ...
                <ul class="mb-4 flex flex-col gap-1">
                    <li>
                        <a href="{{ route('bahanbaku.dashboard') }}" class="{{ request()->routeIs('bahanbaku.dashboard') ? 'active' : '' }}">
                            <button class="cursor-pointer middle none font-sans font-bold center transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 rounded-lg {{ request()->routeIs('bahanbaku.dashboard') ? 'bg-linear-to-tr from-violet-700 to-violet-950 text-white shadow-md shadow-violet-500/20 hover:shadow-lg hover:shadow-violet-500/40' : 'text-white hover:bg-white/10' }} active:opacity-[0.85] w-full flex items-center gap-4 px-4 capitalize" type="button">
                    ...
                    <li>
                        <a href="{{route('bahanbaku.bahanbaku')}}" class="{{ request()->routeIs('bahanbaku.bahanbaku') ? 'active' : ''}}">
                            <button class="cursor-pointer middle none font-sans font-bold center transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 rounded-lg {{ request()->routeIs('bahanbaku.bahanbaku') ? 'bg-linear-to-tr from-violet-700 to-violet-950 text-white shadow-md shadow-violet-500/20 hover:shadow-lg hover:shadow-violet-500/40' : 'text-white hover:bg-white/10' }} active:opacity-[0.85] w-full flex items-center gap-4 px-4 capitalize" type="button">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
